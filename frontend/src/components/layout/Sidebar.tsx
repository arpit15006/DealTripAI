...
import { ArrowRightIcon, ChevronRightIcon, SquareArrowOutUpRightIcon } from 'lucide-react'
...
function isLinkActive(
  href: string,
  activePath: string | RegExp | undefined,
  pathname: string,
  searchParams: Pick<URLSearchParams, 'get'>
): boolean {
  if (activePath) {
    return activePath instanceof RegExp ? activePath.test(pathname) : pathname.startsWith(activePath)
  }
                          ...
                          <Collapsible className='group/subcollapsible' key={subItem.label}>
                            <SidebarMenuSubItem>
                              <CollapsibleTrigger
                                ...
                                render={
                                  <SidebarMenuSubButton
                                    className='data-active:bg-primary/10! data-active:text-primary! data-active:hover:bg-primary/10! data-active:hover:text-primary! justify-between'
...
const SidebarLayout = () => {
  ...
  return (
    <Sidebar
      ...
      className='**:data-[slot=sidebar-inner]:bg-muted **:data-[slot=sidebar-inner]:rounded-t-xl'
    >
      <SidebarHeader>
        <SidebarMenu>
          <SidebarMenuItem>
      ...
      <SidebarFooter className='group-data-[collapsible=icon]:-rotate-90'>
        <Button
          size='sm'
          className='hidden group-data-[collapsible=icon]:flex group-data-[collapsible=icon]:w-fit'
          render={<Link href='/' />}
          nativeButton={false}
        >
          Visit site
        ...
        <Link
          href='/'
          className='group/home-card relative isolate flex h-36 flex-col justify-between overflow-hidden rounded-xl p-4 shadow-sm transition-shadow duration-300 group-data-[collapsible=icon]:hidden hover:shadow-lg'
        >
          <img
            src='/images/bg-landscape.webp'
            alt=''
            className='absolute inset-0 -z-20 size-full object-cover transition-transform duration-500 ease-out group-hover/home-card:scale-110'
          />
